...
class TestTransceiver(unittest.TestCase):
    ...
    def test_decode_packet_start_out_of_bounds(self):
        ...
        with patch.object(self.rx.sync, 'detect', return_value=0) as mock_detect:
             decoded, consumed = self.rx.decode_frame(rx_input)
    ...
    def test_decode_warmup_only_symbol_path(self):
        ...
        with patch.object(self.rx.sync, 'detect', return_value=sync_idx) as mock_detect, \
             patch.object(self.rx, 'filter_signal', side_effect=lambda x: x) as mock_filter, \
             patch.object(self.rx.data_handler, 'decode', side_effect=ValueError("Header Corruption")) as mock_data:
